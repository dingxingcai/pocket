...
//    }
    ...
    public function resolve($root, $args)
    {
        ...
        //每天各门店详细订单
        $dayOrder = DB::connection('sqlsrv')->select("select  CONVERT(varchar(10), b.billdate, 23) as 'date' ,s.FullName as 'storeName',isnull(count(billdate) ,0) as  'totalOrders', SUM( CASE WHEN b.VipCardID!=-1 THEN 1 ELSE 0 END)  as 'vipOrders' ,SUM( CASE WHEN b.VipCardID=-1 THEN 1 ELSE 0 END)   as 'notVipOrders' from BillIndex b left join stock s on s.typeId = b.ktypeid where datediff(dd,BillDate,getdate()) <= 1 and draft = 0  and BillType = 305 and  BillDate < CONVERT(varchar(30),getdate(),23)  and b.RedWord=0  group by BillDate,s.FullName  order by BillDate desc ,'totalOrders' DESC  ; ");
        return $dayOrder;


        //使用订单中心的数据
//        $dayOrder = DB::connection('mysql')->select("select
//DATE_FORMAT(b.billdate,'%Y-%m-%d') as 'date' ,
//s.name as 'storeName',
//IFNULL(count(b.billdate),0) as  'totalOrders',
//SUM( CASE WHEN b.VipCardID!=-1 THEN 1 ELSE 0 END)  as 'vipOrders' ,
//SUM( CASE WHEN b.VipCardID=-1 THEN 1 ELSE 0 END)   as 'notVipOrders'
//from
//BillIndex b LEFT join dim_store s on s.storeId = b.ktypeid
//where  b.BillType = 305
//and b.BillDate = date_sub(curdate(),interval 1 day)
//and b.RedWord=0
//group by b.BillDate,s.name
//order by b.BillDate DESC ,totalOrders DESC  ");
//        return $dayOrder;
    }
}