...
and  b.BillDate <= CONVERT(varchar(30),getdate(),23) and b.draft = 0  group by p.ParID,b.BillDate order by date asc;");
        foreach ($sales as &$sale) {
            //统计当天的总计的销售额
            $totalMoney = DB::connection('sqlsrv')->select("select  sum(TotalInMoney) as 'totalMoney'  from billindex
where  BillType = 305 and draft = 0  and  BillDate = '{$sale->date}';");
